...
Plugin_ShallowWater::Plugin_ShallowWater() :
	...
	iteradapt_(5),
	adaptive_mesh_(true),
    criteria_(Criteria::H_tempo),

    seuil_sub_h(0.5),//1
    seuil_simp_h(0.25),//0.5
    seuil_sub_V(0.5),//0.5
    seuil_simp_V(0.2),//0.5
...
void Plugin_ShallowWater::clean_map()
{
	...
	delete atq_map_;
	atq_map_ = nullptr;
	plugin_cmap_provider_->remove_cmap2("shallow_water_2");
	mh_ = nullptr;
	map_ = nullptr;
}
		...
		do
		{
			if (tag == std::string("NS")) // lecture d'une face
			{
				...
				for (uint32 i = 0; i < vertex_id_vect.size() - 1; ++i)
				{
					...
					map_->foreach_adjacent_vertex_through_edge(v, [&] (CMap2::Vertex av) -> bool
					{
						if (map_->embedding(av) == vertex_id_vect[i+1] + nb_vertices_2d_ - 1)
						{
		...
		do
		{
			if (tag == std::string("NS"))
			{
				...
				for (uint32 i = 0; i < vertex_id_vect.size() - 1; ++i)
				{
					...
					map_->foreach_adjacent_vertex_through_edge(v, [&] (CMap2::Vertex av) -> bool
					{
						if ((int)(map_->embedding(av)) == vertex_id_vect[i+1] - 1)
						{
							NS_[CMap2::Edge(av.dart)] = ns_num;
							ns_edge_found = true;
						}
...
void Plugin_ShallowWater::load_2D_boundary_cond_file(const QString& filename)
{
	...
	if (number_of_BC > 0)
	{
		...
		map_->parallel_foreach_cell(
			[&] (CMap2::Edge e)
			{
				if (!map_->is_incident_to_boundary(e))
					return;
...
void Plugin_ShallowWater::sew_1D_2D_meshes()
{
	map_->foreach_cell(
		[&] (CMap2::Edge e1)
		{
			...
			map_->foreach_cell([&] (CMap2::Edge e2) -> bool
			{
...
void Plugin_ShallowWater::load_project(const QString& dir)
{
	...
	// create map
	mh_ = plugin_cmap_provider_->add_cmap2("shallow_water_2");
	map_ = mh_->map();
	qtrav_ = new CMap2::QuickTraversor(*map_);
	edge_left_side_ = new CMap2::DartMarker(*map_);

	// import 2D mesh
	QString mesh_2D_filepath = dir + "/" + mesh_2D_filename_;
	cgogn::io::import_surface<VEC3>(*map_, mesh_2D_filepath.toStdString());

	dimension_ = map_->add_attribute<uint8, CMap2::Face::ORBIT>("dimension");
	dimension_.set_all_values(2);

	nb_faces_2d_ = map_->nb_cells<CMap2::Face>();
	nb_vertices_2d_ = map_->nb_cells<CMap2::Vertex>();
	...
	if (dim_ == 12)
	{
		...
		cgogn::io::import_surface<VEC3>(map_tmp, mesh_1D_filepath.toStdString());
		map_tmp.enforce_unique_orbit_embedding<CMap2::Vertex::ORBIT>();
		CMap2::DartMarker dm(*map_);
		map_->merge(map_tmp, dm);
		map_->parallel_foreach_cell([&] (CMap2::Face f)
		{
			if (dimension_[f] != 2)
				dimension_[f] = 1;
		});
	...
	mh_->notify_attribute_added(CMap2::Vertex::ORBIT, "position");
	...
	atq_map_ = new cgogn::AdaptiveTriQuadCMap2(*map_);
	atq_map_->init();
	...
	map_->parallel_foreach_cell(
		...
		{
			uint32 nbv = 0;
			map_->foreach_incident_vertex(f, [&] (CMap2::Vertex v)
			{
				zb_[f] += position_[v][2];
				nbv++;
			});
	...
	map_->copy_attribute(water_position_, position_);

	cgogn::geometry::compute_centroid<CMap2::Face>(*map_, position_, centroid_);
	cgogn::geometry::compute_area<CMap2::Face>(*map_, position_, area_);
...
void Plugin_ShallowWater::init()
{
    ...
    logged_values_all_.clear();

    //std::vector<uint32> vertices = {490, 301, 9, 69, 342, 295};
    //for (uint32 v : vertices)
	map_->foreach_cell(
        [&] (CMap2::Vertex v)
    {
		logged_values_[map_->embedding(v)] = {{}, {}, {}};
    },
    *qtrav_
   );
	...
	map_->foreach_cell(
        [&] (CMap2::Vertex v0)
        {
			logged_values_all_[map_->embedding(v0)] = {{}, {}, {}};
        },
        *qtrav_
    );
...
void Plugin_ShallowWater::stop()
{
    ...
    if (t_ == t_max_)
	{
		...
		if (adaptive_mesh_)
		{
			...
			switch (criteria_)
			{
                case Criteria::H_spatial: dir += "_H_spatial"; break;
                case Criteria::Q_R_spatial: dir += "_Q_R_spatial";break;
                case Criteria::H_Q_R_spatial: dir += "_H_Q_R_spatial"; break;

                case Criteria::H_tempo: dir += "_H_tempo_"; break;
                case Criteria::Q_R_tempo: dir+="_Q_R_tempo_";break;
            ...
            dir += QDateTime::currentDateTime().toString("_MM-dd_hh-mm-ss");
		}
		else
		{
			dir += "fixed_" + QString::number(max_depth_);
		}
        //dir += QDateTime::currentDateTime().toString("_MM-dd_hh-mm-ss-zzz"); // la date ne m'interesse pas dans le cas fixe
        ...
        switch (criteria_)
        {
            case Criteria::H_spatial:
                {
                ofs<<"\n seuil de subd h"<<seuil_sub_h;
                ofs<<"\n seuil de simp h"<<seuil_simp_h;
                break;
                }
            case Criteria::Q_R_spatial:
               {
                ofs<<"\n seuil de subd V : "<<seuil_sub_V;
                ofs<<"\n seuil de simp V : "<<seuil_simp_V;
                break;
            }
            case Criteria::H_Q_R_spatial:
               {
                ofs<<"\n seuil de subd h"<<seuil_sub_h;
                ofs<<"\n seuil de simp h"<<seuil_simp_h;
                ofs<<"\n seuil de subd V : "<<seuil_sub_V;
        ...
        filename = outdir.absolutePath() + "/ids.txt";
        QString filename2 = outdir.absolutePath() + "/h.txt";
        QString filename3 = outdir.absolutePath() + "/q.txt";
        QString filename4 = outdir.absolutePath() + "/r.txt";

//        QString filename6 = outdir.absolutePath() + "/x.txt";
//        QString filename7 = outdir.absolutePath() + "/y.txt";


        ofs = std::ofstream(filename.toStdString(), std::ofstream::out);
        std::ofstream ofs2 = std::ofstream(filename2.toStdString(), std::ofstream::out);
        std::ofstream ofs3 = std::ofstream(filename3.toStdString(), std::ofstream::out);
        std::ofstream ofs4 = std::ofstream(filename4.toStdString(), std::ofstream::out);

//        std::ofstream ofs6 = std::ofstream(filename6.toStdString(), std::ofstream::out);
//        std::ofstream ofs7 = std::ofstream(filename7.toStdString(), std::ofstream::out);

       for (const auto& vdata : logged_values_all_)
        {
            uint32 vid= vdata.first;
            SCALAR vh = std::get<0>(vdata.second);
            SCALAR vq = std::get<1>(vdata.second);
            SCALAR vr = std::get<2>(vdata.second);

//            SCALAR vx = std::get<4>(vdata.second);
//            SCALAR vy = std::get<5>(vdata.second);
            ofs<<vid<<',';
            ofs2<<vh<<',';
            ofs3<<vq<<',';
            ofs4<<vr<<',';
...
void Plugin_ShallowWater::update_draw_data()
{
	...
	map_->parallel_foreach_cell(
		[&] (CMap2::Vertex v)
		{
			...
			SCALAR r = 0;
			uint32 nbf = 0;
			map_->foreach_incident_face(v, [&] (CMap2::Face f)
			{
				h += h_[f];
//				h += (h_[f] + zb_[f]);
				q += q_[f];
	...
	mh_->notify_connectivity_change();
	mh_->init_primitives(cgogn::rendering::POINTS);
	mh_->init_primitives(cgogn::rendering::LINES);
	mh_->init_primitives(cgogn::rendering::TRIANGLES);
	mh_->init_primitives(cgogn::rendering::BOUNDARY);

	// notify attribute changes
	mh_->notify_attribute_change(CMap2::Vertex::ORBIT, "scalar_value_h");
	mh_->notify_attribute_change(CMap2::Vertex::ORBIT, "scalar_value_u");
	mh_->notify_attribute_change(CMap2::Vertex::ORBIT, "scalar_value_v");
	mh_->notify_attribute_change(CMap2::Vertex::ORBIT, "position");
	mh_->notify_attribute_change(CMap2::Vertex::ORBIT, "water_position");
	mh_->notify_attribute_change(CMap2::Vertex::ORBIT, "flow_velocity");

	simu_data_access_.unlock();
	mh_->unlock_topo_access();
}

void Plugin_ShallowWater::export_frame()
{
    QString frame_filename = export_frames_dir_ + "frame_" + QString::number(frame_num_) + ".bmp";
	schnapps_->selected_view()->saveSnapshot(frame_filename, true);
    ++frame_num_;
}
...
void Plugin_ShallowWater::update_time_step()
{
	...
	map_->parallel_foreach_cell(
		...
		{
			uint32 fidx = map_->embedding(f);
			map_->foreach_incident_edge(f, [&] (CMap2::Edge ie)
			{
				uint32 ieidx = map_->embedding(ie);
...
//	SCALAR min_dt = std::min(dt_max_, t_max_ - t_);
...
//			if (area_[fidx]*phi_[fidx]*(h_[fidx]+zb_[fidx]) < (-discharge_[fidx]*min_dt))
//				min_dt = - area_[fidx]*phi_[fidx]*(h_[fidx]+zb_[fidx]) / discharge_[fidx];
		},
		*qtrav_
	);
...
//	auto start = std::chrono::high_resolution_clock::now();
	...
	map_->parallel_foreach_cell(
		[&] (CMap2::Edge e)
		{
			uint32 eidx = map_->embedding(e);

			// solve flux on edge
			Str_Riemann_Flux riemann_flux;

			if (map_->is_incident_to_boundary(e)) // border conditions
			{
				CMap2::Face f(e.dart);
				uint32 fidx = map_->embedding(f);
				if (phi_[f] > small_)
			...
			else // Inner cell: use the lateralised Riemann solver
			{
				...
				uint32 f1idx = map_->embedding(f1);
				uint32 f2idx = map_->embedding(f2);
...
//		std::this_thread::sleep_for(sleep_duration);
	...
	nbmailles = map_->nb_cells<CMap2::Face>();
    ...
    if (t_ > 10. && sup10)
    {
        ...
        for (auto& vdata : logged_values_all_)
        {
            uint32 nbf = 0;
            SCALAR h = 0.;
            SCALAR q = 0.;
            ...
            std::get<0>(vdata.second)=h;
            std::get<1>(vdata.second)=q;
            std::get<2>(vdata.second)=r;

            //std::get<4>(vdata.second)=posV[0];
            //std::get<5>(vdata.second)=posV[1];
        }
...
bool Plugin_ShallowWater::subd_criteria_h_spatial(CMap2::Face f)
{
    bool res = false;
	uint32 fidx = map_->embedding(f);
	map_->foreach_adjacent_face_through_edge(f, [&] (CMap2::Face af) -> bool
    {
		uint32 afidx = map_->embedding(af);

        SCALAR diffh = fabs(h_[fidx] - h_[afidx]);
        if (diffh > seuil_sub_h)
        {
            res = true;
            return false;
        }
...
bool Plugin_ShallowWater::subd_criteria_q_r_spatial(CMap2::Face f)
{
    bool res = false;
	uint32 fidx = map_->embedding(f);
	map_->foreach_adjacent_face_through_edge(f, [&] (CMap2::Face af) -> bool
...
bool Plugin_ShallowWater::subd_criteria_h_q_r_spatial(CMap2::Face f)
{
    bool res = false;
	uint32 fidx = map_->embedding(f);
	map_->foreach_adjacent_face_through_edge(f, [&] (CMap2::Face af) -> bool
    {
        ...
        SCALAR diffh = fabs(h_[fidx] - h_[afidx]);
        //if (diffh/chifaa_max_diff_h > seuil_sub)
        if (diffh > seuil_sub_h)
        {
            res = true;
            return false;
        ...
        SCALAR diffq = fabs(q_[fidx] - q_[afidx]);
        //if (diffq/chifaa_max_diff_q > seuil_sub)
        if (diffq > seuil_sub_V)
        {
            res = true;
            return false;
        ...
        SCALAR diffr = fabs(r_[fidx] - r_[afidx]);
        //if (diffr/chifaa_max_diff_r > seuil_sub)
        if (diffr > seuil_sub_V)
        {
            res = true;
            return false;
...
bool Plugin_ShallowWater::subd_criteria_h_tempo(CMap2::Face f)
{
	uint32 fidx = map_->embedding(f);
    if (abs(h_[fidx] - h_old_[fidx])/dt_ > seuil_sub_h)
        return true;
    return false;
}
bool Plugin_ShallowWater::subd_criteria_q_r_tempo(CMap2::Face f)
{
	uint32 fidx = map_->embedding(f);
    if (abs(q_[fidx] - q_old_[fidx])/dt_ > seuil_sub_V ||
        abs(r_[fidx] - r_old_[fidx])/dt_ > seuil_sub_V)
    {
...
bool Plugin_ShallowWater::subd_criteria_h_q_r_tempo(CMap2::Face f)
{
	uint32 fidx = map_->embedding(f);
//    std::cout<<"h: "<<abs(h_[fidx] - h_old_[fidx])/dt_ <<std::endl;
//    std::cout<<"q: "<<abs(q_[fidx] - q_old_[fidx])/dt_<<std::endl;
//    std::cout<<"r: "<<abs(r_[fidx] - r_old_[fidx])/dt_<<std::endl;


    if (abs(h_[fidx] - h_old_[fidx])/dt_ > seuil_sub_h ||
        abs(q_[fidx] - q_old_[fidx])/dt_ > seuil_sub_V ||
        abs(r_[fidx] - r_old_[fidx])/dt_ > seuil_sub_V)
    {
...
bool Plugin_ShallowWater::subd_criteria_angleV_spatial(CMap2::Face f)
{
    bool res = false;
	uint32 fidx = map_->embedding(f);
    SCALAR norm_V_=(q_[fidx]*q_[fidx]+r_[fidx]*r_[fidx])/ (h_[fidx]*h_[fidx]);
	map_->foreach_adjacent_face_through_edge(f, [&] (CMap2::Face af) -> bool
    {
		uint32 afidx = map_->embedding(af);
        SCALAR angle_diff = cgogn::geometry::angle(VEC2(q_[fidx], r_[fidx]), VEC2(q_[afidx], r_[afidx]));

        if (angle_diff > seuil_sub_angleV && abs(norm_V_)>0.05  )
        {
            res = true;
            return false;
        }
...
bool Plugin_ShallowWater::subd_criteria_angleV_tempo(CMap2::Face f)
{
    ...
    SCALAR angle_diff = cgogn::geometry::angle(VEC2(q_[fidx], r_[fidx]), VEC2(q_old_[fidx], r_old_[fidx]));


    SCALAR norm_V_=(q_[fidx]*q_[fidx]+r_[fidx]*r_[fidx])/ (h_[fidx]*h_[fidx]);

    if (//abs(h_[fidx] - h_old_[fidx]) > seuil_sub_h ||
//         angle_diff> seuil_sub_angleV && abs(norm_V_-norm_V_old_)>0.1*abs(norm_V_old_)  )//||
         angle_diff/dt_> seuil_sub_angleV && abs(norm_V_)>0.05  )//||
            //        abs(norm_V_ - norm_V_old_) > seuil_sub_norm)
    {
...
{
	CMap2::CellMarker<CMap2::Face::ORBIT> subdivided(*map_);
...
//		fv = cgogn::dart_buffers()->cell_buffer<CMap2::Face>();
	...
	std::vector<CMap2::Face>* faces_to_subdivide = cgogn::dart_buffers()->cell_buffer<CMap2::Face>();
...
//					faces_to_subdivide_per_thread[cgogn::current_thread_index()]->push_back(f);
					faces_to_subdivide->push_back(f);
            }
            else
            {
//				faces_to_subdivide_per_thread[cgogn::current_thread_index()]->push_back(f);
...
//	{
//		for (CMap2::Face f : *fv)
		for (CMap2::Face f : *faces_to_subdivide)
		{
			if (!subdivided.is_marked(f))
			{
				...
				atq_map_->subdivide_face(f,
					[&] (CMap2::Vertex v)
					{
						if (map_->is_incident_to_boundary(CMap2::Edge(v.dart)))
						{
							typ_bc_[CMap2::Edge(v.dart)] = typ_bc_[CMap2::Edge(map_->phi_1(v.dart))];
							val_bc_[CMap2::Edge(v.dart)] = val_bc_[CMap2::Edge(map_->phi_1(v.dart))];
						}
						position_[v] = (position_[CMap2::Vertex(map_->phi_1(v.dart))] + position_[CMap2::Vertex(map_->phi1(v.dart))]) / 2.;
						water_position_[v] = position_[v];
						compute_edge_length_and_normal(CMap2::Edge(v.dart));
						compute_edge_length_and_normal(CMap2::Edge(map_->phi_1(v.dart)));
						qtrav_->update(v);
						qtrav_->update(CMap2::Edge(v.dart));
						qtrav_->update(CMap2::Edge(map_->phi_1(v.dart)));
					},
					[&] (CMap2::Face f)
					{
						uint32 fidx = map_->embedding(f);
					...
					[&] (CMap2::Face f)
					{
						...
						{
							cgogn::Dart cfd = map_->phi<12>(f.dart);
							CMap2::Face cf(cfd);
							uint32 cfidx = map_->embedding(cf);
							...
							SCALAR zb = 0.;
							uint32 nbv = 0;
							map_->foreach_incident_vertex(cf, [&] (CMap2::Vertex iv)
							{
								zb += position_[iv][2];
								nbv++;
							});
							zb_[cfidx] = zb / nbv;
							centroid_[cfidx] = cgogn::geometry::centroid(*map_, cf, position_);
							area_[cfidx] = cgogn::geometry::area(*map_, cf, position_);
							dimension_[cfidx] = 2;

							map_->foreach_incident_edge(cf, [&] (CMap2::Edge ie)
							{
								qtrav_->update(ie);
								compute_edge_length_and_normal(ie);
							});

							map_->foreach_adjacent_face_through_edge(cf, [&] (CMap2::Face af)
							{
								uint32 afidx = map_->embedding(af);
								qtrav_->update(af);
								subdivided.mark(af);
								...
								SCALAR zb = 0.;
								uint32 nbv = 0;
								map_->foreach_incident_vertex(af, [&] (CMap2::Vertex iv)
								{
									zb += position_[iv][2];
									nbv++;
								});
								zb_[afidx] = zb / nbv;
								centroid_[afidx] = cgogn::geometry::centroid(*map_, af, position_);
								area_[afidx] = cgogn::geometry::area(*map_, af, position_);
								dimension_[afidx] = 2;
							});
						}
						else
						{
							cgogn::Dart cvd = map_->phi<12>(f.dart);
							CMap2::Vertex cv(cvd);
							...
							map_->foreach_incident_edge(cv, [&] (CMap2::Edge ie)
							{
								qtrav_->update(ie);
								compute_edge_length_and_normal(ie);
							});

							map_->foreach_incident_face(cv, [&] (CMap2::Face af)
							{
								uint32 afidx = map_->embedding(af);
								qtrav_->update(af);
								subdivided.mark(af);
...
//		cgogn::dart_buffers()->release_cell_buffer<CMap2::Face>(fv);
	cgogn::dart_buffers()->release_cell_buffer<CMap2::Face>(faces_to_subdivide);
}


bool Plugin_ShallowWater::simp_criteria_h_spatial(cgogn::Dart central_cell)
{
    ...
    else
    {
		...
		map_->foreach_incident_edge(cv, [&] (CMap2::Edge ie)
        {
			...
			CMap2::Face f2(map_->phi2(ie.dart));
            SCALAR diff_h = fabs(h_[f1] - h_[f2]);
            max_diff_h = diff_h > max_diff_h ? diff_h : max_diff_h;
        });
    }

    if (max_diff_h < seuil_simp_h)
        return true;

    return false;
}
bool Plugin_ShallowWater::simp_criteria_q_r_spatial(cgogn::Dart central_cell)
{
    ...
    if (max_diff_q < seuil_simp_V && max_diff_r < seuil_simp_V)
        return true;

    return false;
}
bool Plugin_ShallowWater::simp_criteria_h_q_r_spatial(cgogn::Dart central_cell)
{
    SCALAR max_diff_h = 0.0;
    SCALAR max_diff_q = 0.0;
    SCALAR max_diff_r = 0.0;
    ...
    if (max_diff_h < seuil_simp_h && max_diff_q < seuil_simp_V && max_diff_r < seuil_simp_V)
        return true;

    return false;
}
bool Plugin_ShallowWater::simp_criteria_h_tempo(cgogn::Dart central_cell)
{
    bool res = true;
    CMap2::Face f(central_cell);
    if (atq_map_->is_triangle_face(f))
    {
        CMap2::Face cf(central_cell);
		uint32 cfidx = map_->embedding(cf);
        if (abs(h_[cfidx] - h_old_[cfidx])/dt_ > seuil_simp_h)
            res = false;
		map_->foreach_adjacent_face_through_edge(cf, [&] (CMap2::Face af)
        {
			uint32 afidx = map_->embedding(af);
            if (abs(h_[afidx] - h_old_[afidx])/dt_ > seuil_simp_h)
                res = false;
        });
    }
    else
    {
        CMap2::Vertex cv(central_cell);
		map_->foreach_incident_face(cv, [&] (CMap2::Face iface)
        {
			uint32 ifidx = map_->embedding(iface);
            if (abs(h_[ifidx] - h_old_[ifidx])/dt_ > seuil_simp_h)
                res = false;
        });
    }
    return res;
}
bool Plugin_ShallowWater::simp_criteria_q_r_tempo(cgogn::Dart central_cell)
{
    ...
    if (atq_map_->is_triangle_face(f))
    {
        CMap2::Face cf(central_cell);
		uint32 cfidx = map_->embedding(cf);
        if (abs(q_[cfidx] - q_old_[cfidx]) /dt_> seuil_simp_V ||
            abs(r_[cfidx] - r_old_[cfidx])/dt_ > seuil_simp_V)
        {
        ...
        {
			uint32 afidx = map_->embedding(af);
            if (abs(q_[afidx] - q_old_[afidx])/dt_ > seuil_simp_V ||
                abs(r_[afidx] - r_old_[afidx])/dt_ > seuil_simp_V)
            {
    ...
    else
    {
        ...
        {
			uint32 ifidx = map_->embedding(iface);
            if (abs(q_[ifidx] - q_old_[ifidx]) /dt_> seuil_simp_V ||
                abs(r_[ifidx] - r_old_[ifidx]) /dt_> seuil_simp_V)
            {
...
bool Plugin_ShallowWater::simp_criteria_h_q_r_tempo(cgogn::Dart central_cell)
{
    bool res = true;// attention, on dit l'inverse
    CMap2::Face f(central_cell);
    if (atq_map_->is_triangle_face(f))
    {
		...
		uint32 cfidx = map_->embedding(cf);
        if (abs(h_[cfidx] - h_old_[cfidx])/dt_ > seuil_simp_h ||
            abs(q_[cfidx] - q_old_[cfidx])/dt_ > seuil_simp_V ||
            abs(r_[cfidx] - r_old_[cfidx])/dt_ > seuil_simp_V)
        {
		...
		map_->foreach_adjacent_face_through_edge(cf, [&] (CMap2::Face af)
        {
			uint32 afidx = map_->embedding(af);
            if (abs(h_[afidx] - h_old_[afidx]) /dt_> seuil_simp_h ||
                abs(q_[afidx] - q_old_[afidx])/dt_ > seuil_simp_V ||
                abs(r_[afidx] - r_old_[afidx])/dt_ > seuil_simp_V)
            {
    ...
    else
    {
		...
		map_->foreach_incident_face(cv, [&] (CMap2::Face iface)
        {
			uint32 ifidx = map_->embedding(iface);
            if (abs(h_[ifidx] - h_old_[ifidx])/dt_ > seuil_simp_h ||
                abs(q_[ifidx] - q_old_[ifidx]) /dt_> seuil_simp_V ||
                abs(r_[ifidx] - r_old_[ifidx])/dt_ > seuil_simp_V)
            {
...
bool Plugin_ShallowWater::simp_criteria_angleV_spatial(cgogn::Dart central_cell)
{
    SCALAR max_diff_angle = 0.0;
    ...
    if (atq_map_->is_triangle_face(f))
    {
		...
		map_->foreach_adjacent_face_through_edge(cf, [&] (CMap2::Face af)
        {
            SCALAR diff_angle = cgogn::geometry::angle(VEC2(q_[cfidx], r_[cfidx]), VEC2(q_[af], r_[af]));
            max_diff_angle = diff_angle > max_diff_angle ? diff_angle : max_diff_angle;
        });
    }
    else
    {
		...
		map_->foreach_incident_edge(cv, [&] (CMap2::Edge ie)
        {
            CMap2::Face f1(ie.dart);
			CMap2::Face f2(map_->phi2(ie.dart));
            SCALAR diff_angle = cgogn::geometry::angle(VEC2(q_[f1], r_[f1]), VEC2(q_[f2], r_[f2]));
            max_diff_angle = diff_angle > max_diff_angle ? diff_angle : max_diff_angle;
        });
    }

    if (max_diff_angle < seuil_simp_angleV || abs(norm_V_) < 0.05  )
        return true;
...
bool Plugin_ShallowWater::simp_criteria_angleV_tempo(cgogn::Dart central_cell)
{
    ...
    if (atq_map_->is_triangle_face(f))
    {
        ...
        SCALAR angle_diff = cgogn::geometry::angle(VEC2(q_[cfidx], r_[cfidx]), VEC2(q_old_[cfidx], r_old_[cfidx]));

        SCALAR norm_V_=(q_[cfidx]*q_[cfidx]+r_[cfidx]*r_[cfidx])/ (h_[cfidx]*h_[cfidx]);
//        SCALAR norm_V_old_=(q_old_[cfidx]*q_old_[cfidx]+r_old_[cfidx]*r_old_[cfidx])/ (h_[cfidx]*h_[cfidx]);

        if (//abs(h_[cfidx] - h_old_[cfidx]) > seuil_simp_h ||
//            angle_diff > seuil_simp_angleV || abs(norm_V_-norm_V_old_)>0.1*abs(norm_V_old_) )//||
            angle_diff /dt_> seuil_simp_angleV || abs(norm_V_)>0.05 )//||
                //abs(norm_V_-norm_V_old_) > seuil_simp_norm)
        {
		...
		map_->foreach_adjacent_face_through_edge(cf, [&] (CMap2::Face af)
        {
            ...
            SCALAR angle_diff = cgogn::geometry::angle(VEC2(q_[afidx], r_[afidx]), VEC2(q_old_[afidx], r_old_[afidx]));

//            SCALAR angle_V_=atan(q_[afidx]/r_[afidx]);
            SCALAR norm_V_=(q_[afidx]*q_[afidx]+r_[afidx]*r_[afidx])/ (h_[afidx]*h_[afidx]);
//            SCALAR angle_V_old_=atan(q_old_[afidx]/r_old_[afidx]);
            if (//abs(h_[afidx] - h_old_[afidx]) > seuil_simp_h ||
//                angle_diff > seuil_simp_angleV || abs(norm_V_-norm_V_old_)>0.1*abs(norm_V_old_) )// ||
                 angle_diff/dt_ > seuil_simp_angleV || abs(norm_V_)>0.05 )// ||
                //abs(norm_V_-norm_V_old_) > seuil_simp_norm)
            {
    ...
    else
    {
		...
		map_->foreach_incident_face(cv, [&] (CMap2::Face iface)
        {
            ...
            SCALAR angle_diff = cgogn::geometry::angle(VEC2(q_[ifidx], r_[ifidx]), VEC2(q_old_[ifidx], r_old_[ifidx]));

//            SCALAR angle_V_=atan(q_[ifidx]/r_[ifidx]);
            SCALAR norm_V_=(q_[ifidx]*q_[ifidx]+r_[ifidx]*r_[ifidx])/ (h_[ifidx]*h_[ifidx]);
//            SCALAR angle_V_old_=atan(q_old_[ifidx]/r_old_[ifidx]);

            if (//abs(h_[ifidx] - h_old_[ifidx]) > seuil_simp_h ||
//                angle_diff > seuil_simp_angleV || abs(norm_V_-norm_V_old_)>0.1*abs(norm_V_old_) )//||
                    angle_diff /dt_> seuil_simp_angleV || abs(norm_V_)>0.05 )//||
...
void Plugin_ShallowWater::try_simplification()
{
	...
	CMap2::CellMarker<CMap2::Face::ORBIT> treated(*map_);
			...
			if (atq_map_->is_simplifiable(f))
			{
				...
				switch (atq_map_->face_type(f))
				{
					case cgogn::AdaptiveTriQuadCMap2::TRI_CORNER: {
						central_cell = map_->phi<12>(atq_map_->oldest_dart(f)); // central face
						break;
					}
					...
					case cgogn::AdaptiveTriQuadCMap2::QUAD: {
						central_cell = map_->phi<12>(atq_map_->oldest_dart(f)); // central vertex
						break;
					}
				}
	...
	for (CMap2::Face f : *to_simplify)
	{
		atq_map_->simplify_face(f,
			[&] (CMap2::Face f)
			{
				...
				if (atq_map_->is_triangle_face(f))
				{
					...
					if (atq_map_->face_type(f) == cgogn::AdaptiveTriQuadCMap2::TRI_CORNER)
						cf.dart = map_->phi<12>(atq_map_->oldest_dart(f));
					uint32 cfidx = map_->embedding(cf);
				...
				{
					CMap2::Vertex cv(map_->phi<12>(atq_map_->oldest_dart(f)));
					map_->foreach_incident_face(cv, [&] (CMap2::Face iface)
					{
						uint32 ifaceidx = map_->embedding(iface);
			...
			[&] (CMap2::Face f)
			{
				...
				do
				{
					cgogn::Dart next = map_->phi<11>(it);
				...
				SCALAR phi = 0.;
				SCALAR area = 0.;
				for(unsigned int i = 0; i < old_area.size(); i++)
				{
				...
				r_[fidx] = r / area;
				phi_[fidx] = phi / area;
				area_[fidx] = cgogn::geometry::area(*map_, f, position_);

				SCALAR zb = 0.;
				nb = 0;
				map_->foreach_incident_vertex(f, [&] (CMap2::Vertex iv)
				{
					zb += position_[iv][2];
					nb++;
				});
...
void Plugin_ShallowWater::get_LR_faces(CMap2::Edge e, CMap2::Face& fl, CMap2::Face& fr)
{
	...
	{
		fl.dart = e.dart;
		fr.dart = map_->phi2(e.dart);
	}
	else
	{
		fl.dart = map_->phi2(e.dart);
		fr.dart = e.dart;
	}
}
...
{
	SCALAR l = cgogn::geometry::length(*map_, e, position_);
	length_[e] = l;
	VEC3 vec = position_[CMap2::Vertex(map_->phi2(e.dart))] - position_[CMap2::Vertex(e.dart)];
	normX_[e] = vec[1] / l;
	normY_[e] = -vec[0] / l;
	edge_left_side_->mark(e.dart);
	edge_left_side_->unmark(map_->phi2(e.dart));
}
...
// e2 belongs to a 2D face
{
	...
	CMap2::Vertex v2(map_->phi2(e1.dart));
	CMap2::Vertex v3(e2.dart);
	CMap2::Vertex v4(map_->phi2(e2.dart));
	...
	{
		typ_bc_[e2] = "q";
		map_->sew_faces(e2, e1);
		return false;
	}

	else if (almost_equal(position_[v2], position_[v3]) && is_point_in_segment(position_[v3], position_[v4], position_[v1]))
	{
		CMap2::Vertex v = map_->cut_edge(e2);
		position_[v] = position_[v1];
		qtrav_->update(v);
		qtrav_->update(CMap2::Edge(map_->phi1(e2.dart)));
		typ_bc_[CMap2::Edge(map_->phi1(e2.dart))] = "m2";
		typ_bc_[e2] = "q";
		CMap2::Edge e1_tmp(map_->phi2(map_->phi1(map_->phi2(e1.dart))));
		map_->sew_faces(e2,e1);
		tmp = sew_faces_recursive(e1_tmp, CMap2::Edge(map_->phi1(e2.dart)));
		return false;
	}
	...
	else if (almost_equal(position_[v1], position_[v4]) && is_point_in_segment(position_[v3], position_[v4], position_[v2]))
	{
		...
		qtrav_->update(v);
		qtrav_->update(CMap2::Edge(map_->phi1(e2.dart)));
		typ_bc_[CMap2::Edge(map_->phi1(e2.dart))] = "q";
		CMap2::Edge e1_tmp(map_->phi2(map_->phi_1(map_->phi2(e1.dart))));
		map_->sew_faces(CMap2::Edge(map_->phi1(e2.dart)),e1);
	...
	{
		CMap2::Vertex v = map_->cut_edge(e1);
		position_[v] = position_[v4];
		qtrav_->update(v);
		qtrav_->update(CMap2::Edge(map_->phi1(e1.dart)));
		typ_bc_[e2] = "q";
		CMap2::Edge e2_tmp(map_->phi2(map_->phi_1(map_->phi2(e2.dart))));
		map_->sew_faces(e2,CMap2::Edge(map_->phi1(e1.dart)));
	...
	else if (almost_equal(position_[v1], position_[v4]) && is_point_in_segment(position_[v1], position_[v2], position_[v3]))
	{
		...
		qtrav_->update(v);
		qtrav_->update(CMap2::Edge(map_->phi1(e1.dart)));
		typ_bc_[CMap2::Edge(map_->phi1(e1.dart))] = "m1";
		typ_bc_[e2] = "q";
		CMap2::Edge e2_tmp(map_->phi2(map_->phi1(map_->phi2(e2.dart))));
		map_->sew_faces(e2,e1);
		tmp = sew_faces_recursive(CMap2::Edge(map_->phi1(e1.dart)), e2_tmp);
		return false;
	}
